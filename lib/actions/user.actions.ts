...
import { CreateUserParams, DeleteUserParams, GetAllUsersParams, GetSavedQuestionsParams, GetUserByIdParams, GetUserStatsParams, ToggleSaveQuestionParams, UpdateUserParams } from "@/types/shared";
...
export async function getUsersAnswers(params: GetUserStatsParams){
    try {
        ...
        const userAnswers = await Answer.find({
            author: userId
        }).sort({upvotes:-1}).populate('question', '_id name')
        .skip(skipAmount)
        .limit(pageSize)
        .populate('author', '_id clerkId name picture')
